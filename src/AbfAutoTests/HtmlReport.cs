...
{
    public static void AutoAnalysisImages()
    {
        StringBuilder sb = new();
        foreach (string pngPath in Directory.GetFiles(SampleAbfs.AutoAnalysisFolder, "*.png"))
        {
            string pngFilename = Path.GetFileName(pngPath);
            string abfFilename = Path.GetFileNameWithoutExtension(pngPath).Split("_AbfAuto_")[0];
            string abfPath = Path.Combine(SampleAbfs.Folder, abfFilename) + ".abf";
        ...
        string saveAs = Path.Combine(SampleAbfs.AutoAnalysisFolder, "index.html");
